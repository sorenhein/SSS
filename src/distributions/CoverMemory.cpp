...
void CoverMemory::EastHasShortHonors(
  const unsigned char cards,
  const unsigned char tops1,
  const unsigned char len)
{
  CoverSpec& spec = CoverMemory::add(cards, tops1);
  spec.mode = COVER_LENGTHS_AND_TOPS;
  spec.westLength.set(cards-len, COVER_GREATER_EQUAL);
  ...
  const unsigned char upperTopsIncl)
{
  CoverSpec& spec = CoverMemory::add(cards, tops1);
  spec.mode = COVER_LENGTHS_AND_TOPS;
  spec.westLength.set(cards-upperCardsIncl, cards-lowerCardsIncl, 
    COVER_INSIDE_RANGE);
  spec.westTop1.set(tops1-upperTopsIncl, tops1-lowerTopsIncl, 
    COVER_INSIDE_RANGE);
}
...
void CoverMemory::prepare_3_2()
{
  ...
  CoverMemory::AnHonorIsShort(3, 2, 1);      // Stiff H onside + offside

  CoverMemory::WestHasRangeHonors(3, 2, 2, 3); // HH(x) with West
}
...
void CoverMemory::prepare_4_1()
{
  ...
  CoverMemory::WestIsVoid(4, 1);             // 0=4
...
{
  CoverMemory::WestIsVoid(4, 2);             // 0=4
  CoverMemory::EastIsVoid(4, 2);             // 4=0
  ...
  CoverMemory::WestHasARangeHonor(4, 2, 1, 3); // H(xx) with West
  CoverMemory::WestHasARangeHonor(4, 2, 2, 3); // Hx(x) with West
  CoverMemory::EastHasARangeHonor(4, 2, 2, 3); // Hx(x) with East

  CoverMemory::WestHasRangeHonors(4, 2, 2, 3); // HH(x) with West
  CoverMemory::WestHasRangeHonors(4, 2, 3, 3); // HHx with West
  CoverMemory::WestHasRangeHonors(4, 2, 3, 4); // HHx(x) with West
...
void CoverMemory::prepare_4_3()
{
  ...
  CoverMemory::AnHonorIsShort(4, 3, 1);      // Stiff H onside + offside
  CoverMemory::WestHasGeneralAnd(4, 3, 2, 2, 1, 2);   // HH or Hx with West
  CoverMemory::WestHasGeneralAnd(4, 3, 2, 2, 2, 2);   // Exactly HH with West
  CoverMemory::WestHasGeneralAnd(4, 3, 3, 3, 3, 3);   // Exactly HHH with West

  CoverMemory::WestHasExactTops(4, 3, 3);    // HHH(x) onside
...
{
  CoverMemory::WestIsVoid(5, 1);             // West is void
  ...
  CoverMemory::EastHasRangeHonors(5, 1, 2, 3); // Hx, Hxx offside

  CoverMemory::EastHasGeneralAnd(5, 1, 1, 1, 0, 0); // x offside
...
void CoverMemory::prepare_5_2()
{
  ...
  CoverMemory::WestHasCardRange(5, 2, 0, 3); // West 0 to 3, East 5 to 2

  CoverMemory::WestHasExactTops(5, 2, 2);  // HH any with West
  CoverMemory::EastHasExactTops(5, 2, 2);  // HH any with East

  CoverMemory::WestHasShortHonors(5, 2, 3);  // West HH, HHx
  CoverMemory::WestHasShortHonors(5, 2, 4);  // Just not East void

  CoverMemory::WestHasARangeHonor(5, 2, 1, 3); // West has H stiff/2nd/3rd
  CoverMemory::WestHasARangeHonor(5, 2, 2, 3); // West has H 2nd/3rd
  CoverMemory::EastHasARangeHonor(5, 2, 1, 2); // East has H stiff/2nd
  ...
  CoverMemory::EastHasGeneralAnd(5, 2, 1, 1, 0, 0); // x offside
...
{
  CoverMemory::EastIsVoid(5, 3);             // East is void

  CoverMemory::WestHasCardRange(5, 3, 1, 4); // 1-4 cards each
  CoverMemory::WestHasCardRange(5, 3, 2, 3); // 2-3 cards each
  CoverMemory::WestHasCardRange(5, 3, 2, 5); // 2-5 cards with West

  CoverMemory::AnHonorIsShort(5, 3, 1);      // Stiff H onside + offside
...
{
  CoverMemory::EastIsVoid(5, 4);             // East is void
...
{
  CoverMemory::WestHasCardRange(5, 5, 1, 4); // 1-4 cards each
  CoverMemory::WestHasCardRange(5, 5, 2, 3); // 2-3 cards each
}
...
void CoverMemory::prepare_6_1()
{
  ...
  CoverMemory::EastHasExactTops(6, 1, 1);    // H with East, any length

  CoverMemory::WestHasShortHonors(6, 1, 2);  // H or Hx onside
  CoverMemory::WestHasShortHonors(6, 1, 3);  // H, Hx or Hxx onside

  CoverMemory::WestHasRangeHonors(6, 1, 3, 3); // Hxx with West
  CoverMemory::EastHasRangeHonors(6, 1, 1, 5); // H(xxxx) with East
...
void CoverMemory::prepare_6_2()
{
  ...
  CoverMemory::WestHasRangeHonors(6, 2, 3, 3); // HHx with West
  CoverMemory::EastHasRangeHonors(6, 2, 3, 3); // HHx with East

  CoverMemory::AnHonorIsShort(6, 2, 1);      // Stiff H onside + offside
  CoverMemory::HonorsAreShort(6, 2, 2);      // Stiff HH onside + offside

  CoverMemory::WestHasGeneralAnd(6, 2, 2, 5, 1, 1); // Hx(xxx) with West
  CoverMemory::WestHasGeneralAnd(6, 2, 4, 6, 2, 2); // HHxx(xx) with West

  CoverMemory::WestHasGeneralAnd(6, 2, 2, 2, 1, 1); // Hx with West
  CoverMemory::WestHasGeneralAnd(6, 2, 2, 3, 2, 2); // HH(x) with West
}
...
void CoverMemory::prepare_6_4()
{
  ...
  CoverMemory::AnHonorIsShort(6, 4, 1);      // Stiff H onside + offside
}
...
void CoverMemory::prepare_7_2()
{
  ...
  CoverMemory::WestHasExactTops(7, 2, 2);    // HH(xxxxx) with West

  CoverMemory::WestHasRangeHonors(7, 2, 2, 3); // HH(x) with West
  CoverMemory::EastHasRangeHonors(7, 2, 2, 3); // HH(x) with East

  CoverMemory::AnHonorIsShort(7, 2, 1);      // Stiff H onside + offside
  CoverMemory::HonorsAreShort(7, 2, 2);      // H or Hx onside + offside

  CoverMemory::WestHasGeneralAnd(7, 2, 1, 2, 1, 1); // H(x) with West
  CoverMemory::WestHasShortHonors(7, 2, 2); // HH stiff with West
}


void CoverMemory::prepare_7_3()
{
  CoverMemory::WestHasGeneralAnd(7, 3, 3, 3, 3, 3); // HHH with West
  CoverMemory::EastHasGeneralAnd(7, 3, 3, 3, 3, 3); // HHH with East
}
...
{
  CoverMemory::AnHonorIsShort(8, 2, 1);      // Stiff H onside + offside
  CoverMemory::HonorsAreShort(8, 2, 2);      // Stiff HH onside + offside
}
